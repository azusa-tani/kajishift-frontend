...
> 2026年6月11日追記: 本書には旧決済フロー（`POST /api/payments`、`paymentMethod`、`processPayment` 等）を前提にしたテスト記述が含まれます。現行コード上は Stripe β / PaymentIntent 系の実装が存在するため、決済テストはフロントの呼び出し元、バックエンドルート、Webhook、PSP設定を確認してから実施してください。
...
- 決済は外部決済サービス連携の有無が要確認であり、現行コード上は Stripe β運用中の導線を含むため、テストカード・Webhook・冪等性・本番キー利用有無を確認する。
