...
> 2026年6月11日追記: 本DFDは過去時点のフロントエンド構成を元にしています。現行コード上はワーカー審査テスト画面と Stripe β / PaymentIntent 系の決済実装が存在するため、該当フローは再作図が必要です。旧 `processPayment` 前提の対応表は参考情報として扱ってください。
